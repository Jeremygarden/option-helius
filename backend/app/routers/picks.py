...
def _response(tickers: Optional[List[str]] = None) -> Dict[str, Any]:
...
async def scan_single_ticker(ticker: str):
    try:
        from ..services.strategy_scorer import scan_ticker
        return scan_ticker(ticker.upper())
    except Exception as exc:
        return {"ticker": ticker.upper(), "error": str(exc), "fallback": _response([ticker.upper()])}
