...
INDICATOR_CONFIG = {
    ...
    "naaim_exposure": {
        ...
        "data_source": "naaim_csv",
        "ttl_seconds": 86400 * 7,
        "fetch_fn": "fetch_naaim_exposure",
        "weight": 0.03,
        "staleness_warning_hours": 25 * 8,
    },
    "fear_greed": {
    ...
    "sectors_200dma": {
        "name": "Sectors Above 200DMA",
        "tier": RefreshTier.DAILY,
        "data_source": "yfinance",
        "ttl_seconds": 3600,
        "fetch_fn": "fetch_sectors_200dma",
        "weight": 0.05,
        "staleness_warning_hours": 25,
    },
    "skew": {
...
class IndicatorRefreshService:
    ...
    async def fetch_yield_curve(self):
        ...
        if val is None:
            ...
            val = (tnx - irx) if tnx is not None and irx is not None else None
        return {"value": val or 0.0, "source": "FRED" if val is not None else "fallback"}
    ...
    async def fetch_trend(self):
        try:
            ...
            if not hist.empty:
                ...
                if ma200:
                    return {"value": round(((close - ma200) / ma200) * 100, 2), "source": "yfinance"}
        except Exception as exc:
            logger.warning("trend fetch failed: %s", exc)
        return {"value": 0.0, "source": "error"}

    async def fetch_erp(self):
        spx = await fetch_yfinance("^GSPC")
        ten_year = await fetch_fred("GS10")
        if spx is not None and ten_year is not None:
            return {"value": round((100 / 22.0) - float(ten_year), 2), "source": "computed"}
        return {"value": 0.0, "source": "error"}
    ...
    async def fetch_gold_copper(self):
        ...
        if gold and copper:
            return {"value": round(gold / copper, 2), "source": "yfinance"}
        return {"value": 0.0, "source": "error"}
    ...
    async def fetch_rsp_spy(self):
        ...
        if rsp and spy:
            return {"value": round(rsp / spy, 4), "source": "yfinance"}
        return {"value": 0.0, "source": "error"}
        ...
        try:
            ...
            for symbol in tickers:
                ...
                if not hist.empty:
                    close = float(hist["Close"].iloc[-1])
                    ma200 = float(hist["Close"].rolling(200).mean().iloc[-1])
                    above += 1 if close > ma200 else 0
            return {"value": round((above / len(tickers)) * 100, 2), "source": "yfinance"}
        except Exception as exc:
            logger.warning("sector 200dma fetch failed: %s", exc)
        return {"value": 0.0, "source": "error"}
    ...
    async def fetch_fear_greed(self):
        ...
        if data and data.get("data"):
            return {"value": float(data["data"][0]["value"]), "source": "alternative.me"}
        return {"value": 0.0, "source": "error"}
    ...
    async def fetch_put_call_ratio(self):
        ...
        return {"value": float(val) if val is not None else 0.8, "source": "CBOE" if val is not None else "fallback"}
    ...
    async def fetch_naaim_exposure(self):
        ...
        return {"value": float(val) if val is not None else 60.0, "source": "NAAIM" if val is not None else "fallback"}
    ...
    async def fetch_pe_gap(self):
        try:
            ...
            if ttm_pe and fwd_pe:
                return {"value": round(((ttm_pe - fwd_pe) / fwd_pe) * 100, 2), "source": "yfinance"}
        except Exception as exc:
            logger.warning("pe gap fetch failed: %s", exc)
    ...
    async def fetch_cape(self):
        ...
        if val is None:
            val = await fetch_http_csv("https://www.multpl.com/shiller-pe/table/by-month", col_index=1, row_index=1)
        return {"value": float(val) if val is not None else 30.0, "source": "FRED" if val is not None else "fallback"}
