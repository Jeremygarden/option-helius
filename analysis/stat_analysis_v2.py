...
INDICATOR_NAMES = list(SIGMOID_PARAMS_DEFAULTS.keys())
...
def compute_weighted_score(weights, ind_values):
    score = 0
    # Weights are already normalized by constraints
    for i, ind_id in enumerate(INDICATOR_NAMES):
        ...
        score += s_score * weights[i]
    return score

def compute_mse(weights, events, indicators):
    total_error = 0
    for event in events:
    ...
    return total_error / len(events)

def objective_regularized(weights, events, indicators, lambda_reg=0.5):
    mse = compute_mse(weights, events, indicators)
    l2_penalty = lambda_reg * sum(w**2 for w in weights)
    return mse + l2_penalty

def optimize_weights(events, indicators, lambda_reg=0.5):
    n_ind = 18
    bounds = [(0.02, 0.12)] * n_ind
    constraints = [{"type": "eq", "fun": lambda w: sum(w) - 1.0}]
    
    best_result = None
    for seed in range(20):
        np.random.seed(seed)
        w0 = np.random.dirichlet(np.ones(n_ind))
        res = minimize(objective_regularized, w0, args=(events, indicators, lambda_reg), 
                       method='SLSQP', bounds=bounds, constraints=constraints)
        if best_result is None or res.fun < best_result.fun:
            best_result = res
    return best_result.x

def time_series_cv(events, indicators, lambda_reg=0.5):
    cutpoints = ["1960-01", "1975-01", "1990-01", "2005-01"]
    results = []
    for i, cut in enumerate(cutpoints):
        train = [e for e in events if e["date"] < cut]
        test = [e for e in events if e["date"] >= cut and (i == len(cutpoints)-1 or e["date"] < cutpoints[i+1])]
        if len(train) < 5 or len(test) < 2:
            continue
        weights = optimize_weights(train, indicators, lambda_reg)
        train_mse = compute_mse(weights, train, indicators)
        test_mse = compute_mse(weights, test, indicators)
        results.append({
            "fold": i+1,
            "train_period": f"1929-{cut[:4]}",
            "test_period": cut[:4] + "-" + (cutpoints[i+1][:4] if i < len(cutpoints)-1 else "2026"),
            "train_n": len(train),
            "test_n": len(test),
            "train_mse": float(train_mse),
            "test_mse": float(test_mse),
            "weights": dict(zip(INDICATOR_NAMES, [float(w) for w in weights]))
        })
    return results

def ensemble_weights(cv_results):
    fold_weights = [1.0 / r["test_mse"] for r in cv_results]
    total = sum(fold_weights)
    final_weights = {}
    for ind in INDICATOR_NAMES:
        final_weights[ind] = sum(
            r["weights"][ind] * fw / total 
            for r, fw in zip(cv_results, fold_weights)
        )
...
def run_v2_analysis():
    ...
    for l in lambdas:
        weights = optimize_weights(train_events, indicators, lambda_reg=l)
        train_mse = compute_mse(weights, train_events, indicators)
        test_mse = compute_mse(weights, test_events, indicators)
        if test_mse < best_test_mse:
            best_test_mse = test_mse
            best_lambda = l
            reg_results = {
                "lambda_tested": lambdas,
                "best_lambda": l,
                "train_mse": float(train_mse),
                "test_mse": float(test_mse),
    ...
    # 2. 4-fold CV
    cv_results = time_series_cv(events, indicators, lambda_reg=best_lambda)
    
    # 3. Ensemble
    final_ensemble = ensemble_weights(cv_results)
    
    # Weight stability
    weight_std = {}
    for ind in INDICATOR_NAMES:
        weights_across_folds = [r["weights"][ind] for r in cv_results]
        weight_std[ind] = float(np.std(weights_across_folds))
        
    # Comparison
    v1_sparse = {
        "move": 0.25, "dxy": 0.19, "m7_concentration": 0.197, "pe_gap": 0.167,
        "others": 0.01
    }
    
    improvement = (161.1 - reg_results["test_mse"]) / 161.1 * 100
    
    full_results = {
        "regularization": reg_results,
        "cross_validation": {
            "folds": cv_results,
            "avg_train_mse": float(np.mean([r["train_mse"] for r in cv_results])),
            "avg_test_mse": float(np.mean([r["test_mse"] for r in cv_results])),
            "weight_std": weight_std
        },
        "ensemble_weights": final_ensemble,
        "comparison": {
            "v1_sparse": v1_sparse,
            "v2_regularized": reg_results["weights"],
            "improvement_pct": float(improvement)
        }
    }
    
    with open("analysis/analysis_results_v2.json", "w") as f:
